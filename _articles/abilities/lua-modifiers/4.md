...
Without further delay, let's get to it! `npc_abilities_custom.txt` entry for our ability.
...
I decided to go with soul collecting tree: if enemy hero is killed in radius(you need to add this to special values of your `npc_abilities_custom.txt`) the owner of the spell steals up to 2 of the victim's primary stat.
