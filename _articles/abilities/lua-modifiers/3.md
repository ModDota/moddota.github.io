...
function transformation_mod:OnCreated( kv )
    if IsServer() then
        ...
        --We get original attack range and subtract away the wanted attack range, then negate the result.
        --YAY Math!
...
What we want the transformation to affect should go here, these are some of the commons.
...
Now we need to implement these functions. The first is for the model, the second is for the attack range, and the last is for the scale.
...
That's pretty much it. You can add particle effects and/or animations. Though as a note there are some issues with animations on some heroes. For example phantom lancer's death animation makes the player model vanish so I suggest being careful with chosen animation.
